...
contract wBTCCRVStrategy {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    address public constant curverenPool = address(0x93054188d876f558f4a66B2EF1d97d16eDf0895B);
    address public constant mainAsset = address(0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599); //wBTC
    ...
    function lpTokenBalanceChildVault() public returns(uint256){
        return (IDebaVault(crvlpVault).getFullSharePrice()).mul(IERC20(crvlpVault).balanceOf(address(this))).div(1e18);
    }

    function underlyingInChildVault() public returns(uint256){
    ...
    function _withdrawStrategy(uint256 _amount) internal returns(uint256) {
        ...
        if(_lpWithdrawn > 0){
            uint256 _otherAssetBefore = IERC20(renBTC).balanceOf(address(this));
            ...
            uint256 _otherWithdrawn = (IERC20(renBTC).balanceOf(address(this))).sub(_otherAssetBefore);
            if(_otherWithdrawn > 0){
                IERC20(renBTC).safeApprove(curverenPool, 0);
                IERC20(renBTC).safeApprove(curverenPool, _otherWithdrawn);
                ICurveFi(curverenPool).exchange(0, 1, _otherWithdrawn, 0);
            }
        }
    ...
    function _withdrawStrategyAll() internal {
        ...
        if(_lBal > 0){
            ...
            uint256 _garbage = IERC20(renBTC).balanceOf(address(this));
            if(_garbage > 0){
                IERC20(renBTC).safeApprove(curverenPool, 0);
                IERC20(renBTC).safeApprove(curverenPool, _garbage);
                ICurveFi(curverenPool).exchange(0, 1, _garbage, 0);
            }
        }
    }
    ...
    function deposit() public {
        ...
        if(_balance > 0){
            ...
            ICurveFi(curverenPool).add_liquidity([uint256(0), _balance], 0); // Supply wBTC
        }
